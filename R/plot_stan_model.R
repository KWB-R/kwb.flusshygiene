...
plot_stan_model <- function(timestamp, predict, linpred, log = FALSE, q90, q95, 
  ...
  for(j in seq_along(q95)){
    if (q90[j] > 900)
      graphics::rect(xleft = timestamp[j], xright = timestamp[j +1],density = -1, 
                     border = NA, ybottom = 0, ytop = 100000, col = assessment$bad )
    
    if (q90[j] < 900 & q95[j] > 1000)
      graphics::rect(xleft = timestamp[j], xright = timestamp[j+1], ybottom = 0, 
