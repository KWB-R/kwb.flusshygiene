...
plot_data_overview <- function (riverdata, type = "segment") {
  ...
  overview <- dplyr::bind_rows(lapply(riverdata, function(df) {
    ...
    return(dplyr::summarise(
      df2,
      start = range(.data$datum)[1],
      end = range(.data$datum)[2])
    )

  }), .id = "Typ")

  p <- ggplot2::ggplot(overview, ggplot2::aes_string(y = "site", color = "Typ")) +
  ...
  if(type == "segment") {
    ...
    p + ggplot2::geom_segment(size = 4, ggplot2::aes_string(
      x = "start", xend = "end", yend = "site"
    ))
    ...
    p + ggplot2::geom_point(size = 2, alpha = 0.5, ggplot2::aes_string(x = "datum"))
  }
