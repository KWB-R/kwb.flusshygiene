...
get_latest_season <- function(newdata){
  ...
  is_season <- lubridate::year(newdata$datum) == lubridate::year(utils::tail(newdata$datum[is_summer], n=1))
  season <- stats::na.omit(newdata[is_season & is_summer,])
  return(season)
}
...
plot_predicted_quality <- function(model, prediction, ...){
  ...
  plot_stan_model(timestamp = seasonyear$datum, predict = predictyear,
                  linpred = linpredyear, q90 = q90, q95 = q95, ylim = ylim,
                  xlab = xlab, cex = 1.5, log = log)
}
