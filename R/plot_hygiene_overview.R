...
plot_hygiene_overview <- function (hygiene_df)
{
  ...
  graphics::hist(
    log_e.coli, prob = TRUE, breaks = seq(0, 5, 0.25), ylim = c(0, 1),
    right = FALSE, ylab = "", main = "Verteilung E.Coli", col = "#E69F00"
  )

  graphics::text(0.5, 0.5, labels = paste("n =", length(log_e.coli)))

  graphics::curve(add = T, function(x) {

    stats::dnorm(x, mean = mean(log_e.coli), sd = stats::sd(log_e.coli))
  })

  graphics::box()

  graphics::curve(ylab = "", xlim = c(0, 5), function(x) {

    stats::dnorm(x, mean = coef[1], sd = coef[2])
  })

  perz90 <- round(10^(coef[1] + 1.282 * coef[2]))
  perz95 <- round(10^(coef[1] + 1.650 * coef[2]))
  ...
  graphics::boxplot(
    log_e.coli, horizontal = TRUE, main = "", ylim = c(0, 5), col = "#E69F00"
  )
