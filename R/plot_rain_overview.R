...
plot_rain_overview <- function(df)
{
  ...
  df <- dplyr::mutate(df , mean = rowMeans(df[, -1], na.rm = TRUE))

  df <- dplyr::group_by(df, month = lubridate::month(.data$datum, TRUE))

  n_years <- length(unique(format(df$datum, "%Y")))

  df2 <- dplyr::summarise_(
    df, .dots = stats::setNames(
      paste0("sum(", c(site_names, "mean"),", na.rm = T)/", n_years),
      c(site_names, "mean")
    )
  )
  ...
  df2 <- dplyr::mutate(
    df2, site = factor(.data$site, levels = c(site_names, "mean"))
  )

  ggplot2::ggplot(data = df2, ggplot2::aes_string(
    x = "month", y = "Niederschlag"
  )) +
    ...
    ggplot2::ggtitle(paste0(collapse = " - ", c(
      "Durchschnittlicher Monatsniederschlag",
      format(range(df$datum, na.rm = T), "%Y")
    )))
}
